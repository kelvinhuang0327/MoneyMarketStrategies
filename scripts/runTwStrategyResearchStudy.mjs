...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (
      key === "direct-return-linear-challenger"
      || key === "direct-return-challenger"
...
async function main() {
  ...
  if (args.directReturnLinearChallenger) {
    if (args.challengerTemporal || args.balancedLogisticChallenger || args.gnbChallenger || args.returnHurdleLogisticChallenger) {
      throw new Error("STOP_MMS_0056_DIRECT_RETURN_LINEAR_CHALLENGER_MODE_MIXED");
    }
    if (args.roundTripCostBps && args.roundTripCostBps.length > 0) {
      throw new Error("STOP_MMS_0056_DIRECT_RETURN_LINEAR_COST_OVERRIDE_UNSUPPORTED");
    }
    ...
    if (validated.dateRange.max !== args.dataEndDate) {
      throw new Error(
        `STOP_MMS_0056_DIRECT_RETURN_LINEAR_DATA_AS_OF_MISMATCH:expected=${args.dataEndDate}:actual=${validated.dateRange.max}`,
      );
    }
    ...
    const temporalRawRows = rawRows.map((row) => {
      ...
      if (marketRow === undefined) {
        throw new Error(`STOP_MMS_0056_DIRECT_RETURN_LINEAR_ADJUSTED_ROW_MISSING:${row.date}`);
      }
      ...
      if (adjustedRow === undefined) {
        throw new Error(`STOP_MMS_0056_DIRECT_RETURN_LINEAR_ADJUSTED_ROW_UNRESOLVED:${row.date}`);
      }
      return {
        ...row,
    ...
    const temporalResult = runPerSymbolDirectReturnLinearChallengerTemporal({
    ...
    const jsonFile = path.join(
      args.outDir,
      "mms_0056_direct_return_linear_regression_challenger_temporal_v1.json",
    );
    ...
    console.log("DIRECT_RETURN_CHALLENGER_CONCLUSION=" + temporalResult.directReturnChallengerConclusion);
    console.log("PREDICTIVE_SIGNAL=" + temporalResult.doesDirectReturnModelShowPredictiveSignal);
    console.log("ECONOMIC_EVIDENCE=" + temporalResult.doesDirectReturnModelImproveEconomicEvidence);
